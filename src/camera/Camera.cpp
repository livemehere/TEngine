...
glm::mat4 Camera::getProjectionMatrix(const WindowSize &size) {
    ...
    if (std::holds_alternative<PerspectiveProjection>(projection)) {
        ...
        projectionMatrix = glm::perspective(glm::radians(perspective.fov),static_cast<float>(size.w)/ static_cast<float>(size.h), perspective.near, perspective.far);
    } else {
        ...
        projectionMatrix = glm::ortho(-halfWidth,halfWidth,-halfHeight,halfHeight, orthographic.near, orthographic.far);
    }
